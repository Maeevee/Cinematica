...
const SaveDeleteButton: React.FC<Props> = ({ film, className }) => {
const [isSaved, setIsSaved] = useState(() => {
    const storage = localStorage.getItem('favourite');
    const data: { id: number }[] = JSON.parse(storage || '[]');
    return data.some(item => item?.id === film.id);
});

const handleSaveDeleteClick = () => {
    const storage = localStorage.getItem('favourite');
    const data: { id: number }[] = JSON.parse(storage || '[]');
    const isInStorage = data.some(item => item?.id === film.id);
...
return (
    <button onClick={handleSaveDeleteClick} className={className}>
    {isSaved ? (
